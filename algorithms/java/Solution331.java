 ...
 * null 指针的 '#' 。 你可以认为输入格式总是有效的，例如它永远不会包含两个连续的逗号，比如 "1,,3" 。
 * 
...
public class Solution331 {
	public boolean isValidSerialization(String preorder) {
		if(preorder.charAt(0) == '#' && preorder.length() > 2) return false;
		if(preorder.charAt(preorder.length() - 1) != '#') return false;
		int a = 0, b = 0, e;
		while(b < preorder.length() - 2) {
			e = preorder.indexOf(',', b);
			a += "#".equals(preorder.substring(b, e)) ? -1 : 1;
			if(a < 0) return false;
			b = e + 1;
		}
		return a == 0;
